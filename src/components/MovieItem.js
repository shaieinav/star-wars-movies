...
const MovieItem = ({ movie, movieTitle }) => {
    ...
    const getMovieImg = (movieId) => {
        if (movieId === 1) { return thePhantomMenaceImg; }
        if (movieId === 2) { return attackOfTheClonesImg; }
        if (movieId === 3) { return revengeOfTheSithImg; }
        if (movieId === 4) { return aNewHopmImg; }
        if (movieId === 5) { return theEmpireStrikesBackImg; }
        if (movieId === 6) { return returnOfTheJediImg; }
    };
    ...
    return (
        <article id={movie.episode_id} className="movie" onClick={onMovieClick}>
            ...
            <header className="movie-header">
                <img src={getMovieImg(movie.episode_id)} alt="star wars movie" className="movie-img"></img>
            </header>
